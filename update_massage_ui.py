...
# Note: Time slots might vary, I'll try to preserve them if possible or use the common ones.
# Actually, the user updated time slots in previous turn.
# luna: 12:30, 14:30, 16:30, 19:30
# boraspa: 12:30, 14:30, 17:00, 19:30
# spa: 12:30, 14:30, 16:30, 18:30, 19:30
# helios: 09:30, 13:00, 16:30, 19:30
# aspa: 12:30, 14:30, 16:30, 18:30, 19:30
...
for filename, spa_name, options in massage_files:
    ...
    # Replace the reservation-box content
    # I'll look for <!-- 마사지 종류 선택 --> and replace until the end of reservation-box
    ...
    new_box_part = f'''        <!-- 마사지 종류 선택 -->
        <div class="selection-container">
            ...
            <div id="type-selectors" style="display: flex; flex-direction: column; gap: 10px;">
...
{time_tabs_html}
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; padding:12px; background:#f8f9fa; border-radius:15px; border:1px solid #eee;">
            <span style="font-weight:800; font-size:15px; color:#111;">총 예약 인원</span>
            <div style="font-size:18px; font-weight:900; color:#ff6a00;"><span id="total-count-display">0</span>명</div>
        </div>
    ...
    <div class="selection-container">
        <span class="selection-label">마사지 종류 및 인원 선택</span>
        <div id="m-type-selectors" style="display: flex; flex-direction: column; gap: 10px;">
            <!-- JS will render individual counters here -->
        </div>
    </div>
...
{m_time_tabs_html}
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px; background:#f8f9fa; border-radius:15px; margin-bottom:12px; border:1px solid #eee;">
        <span style="font-weight:800; color:#111;">총 예약 인원</span>
        <div style="font-size:18px; font-weight:900; color:#ff6a00;"><span id="m-total-count-display">0</span>명</div>
    </div>
    ...
    function initSelectors() {{
        ...
        containers.forEach(container => {{
            ...
            massageOptions.forEach(opt => {{
                ...
                row.style.cssText = 'display:flex; justify-content:space-between; align-items:center; padding:8px 12px; background:#fcfcfc; border-radius:12px; border:1px solid #f0f0f0;';
                row.innerHTML = `
                    <div style="display:flex; flex-direction:column;">
                        <span style="font-size:14px; font-weight:700; color:#333;">${{opt.name}}</span>
                        <span style="font-size:12px; color:#ff6a00; font-weight:600;">₩ ${{opt.price.toLocaleString()}}</span>
                    </div>
                    <div class="counter">
                        <button class="count-btn" onclick="updateQty('${{opt.name}}', -1)">-</button>
                        <strong class="qty-display" data-name="${{opt.name}}">0</strong>
                        <button class="count-btn" onclick="updateQty('${{opt.name}}', 1)">+</button>
                    </div>
                `;
    ...
    content = re.sub(r'<script>\s*let adultCount = 1;.*?</script>', script_part, content, flags=re.DOTALL)
