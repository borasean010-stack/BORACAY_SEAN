...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTable() {
        ...
        filtered.forEach((res, index) => {
            ...
            tr.innerHTML = `<td><input type="checkbox"></td><td style="color:#bbb;">${filtered.length - index}</td><td style="font-weight:700;">${res.reservationNumber || '-'}</td><td><div style="font-size:14px; font-weight:700; white-space:nowrap;">${res.customerKorName || '미입력'} <span style="color:#888; font-weight:400; font-size:12px; margin-left:4px;">(${res.engName || '-'})</span></div></td><td style="font-weight:600;">${itemsText}</td><td style="font-weight:800;">₩ ${(res.totalPrice || 0).toLocaleString()}</td><td style="color:#888;">${res.createdAt?.toDate ? res.createdAt.toDate().toLocaleDateString() : '-'}</td><td style="text-align:center;"><span class="n-badge ${status === '예약확정' || status === '리조트확정' ? 'badge-green' : (status === '견적' || status === '견적완료' ? 'badge-blue' : 'badge-yellow')}">${status}</span></td><td><div style="display:flex; gap:5px;">${actionButtons}</div></td>`;
