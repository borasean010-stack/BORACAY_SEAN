...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.switchAdminTab = (tab) => {
        ...
        if (tab === 'system') { 
            ...
            if(statusLayer) statusLayer.style.display = 'none'; 
            if(dataViewSection) dataViewSection.style.display = 'block'; 
            ...
            if (bActive) bActive.innerText = '시스템 설정';
        }
        else { 
            if(scheduleSection) scheduleSection.style.display = 'block'; 
            if(statusLayer) statusLayer.style.display = 'flex'; 
            if(dataViewSection) dataViewSection.style.display = 'block'; 
            ...
            const bActive = document.getElementById('breadcrumb-active');
            if (bActive) {
                bActive.innerText = tab === 'new' ? '신규예약' : tab === 'confirmed' ? '예약확정' : tab === 'resorts' ? '리조트 견적' : '리조트 확정';
            }
        }
        renderTable();
    };
    ...
    window.makeQuickVoucher = async () => {
        ...
        const pickupResort = translateResort(parts[9] || '');
        ...
        const paxDetailStr = `성인 ${pax}, 아동 ${chd}, 유아 ${inf} (총 ${totalPaxCount}명)`;
        const items = [];

        if (flightIn.match(/[A-Z]{2}\d{2,}/) && checkIn.includes('/')) {
            const dp = checkIn.split('/');
            items.push({ name: `✈️ 공항 픽업 (${flightIn})`, date: `${currentYear}-${dp[0].padStart(2,'0')}-${dp[1].trim().padStart(2,'0')}`, time: " ", count: totalPaxCount, details: `리조트 : ${pickupResort}` });
        }
        ...
        if (fullRemarks) {
            fullRemarks.split('\n').forEach(line => {
                ...
                if (dm) {
                    ...
                    let itemTime = tm ? `${tm[1].padStart(2,'0')}:${tm[2]}` : "";
        ...
        if (flightOut.match(/[A-Z]{2}\d{2,}/) && checkOut.includes('/')) {
            const dp = checkOut.split('/');
            items.push({ name: `✈️ 공항 샌딩 (${flightOut})`, date: `${currentYear}-${dp[0].padStart(2,'0')}-${dp[1].trim().padStart(2,'0')}`, time: "21:00", count: totalPaxCount, details: `리조트 : ${pickupResort}` });
        }
        ...
        try {
            ...
            if (confirm(`퀵바우처 생성 완료!\n링크를 복사하시겠습니까?`)) { navigator.clipboard.writeText(url).then(() => alert('복사되었습니다.')); }
