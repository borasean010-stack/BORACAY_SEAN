...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMDProducts() {
        ...
        mdRecommendedTitles.forEach(title => {
            ...
            div.onclick = () => { window.location.href = p.url + "?v=1.4"; };
            div.innerHTML = `
                ${getBadgesHtml(p)}
                <div class="img-container card-img-wrap"><img src="${p.img}?v=1.4" alt="${p.title}" loading="lazy"></div>
                <div class="card-body">
                    <h3>${p.title}</h3>
    ...
    function renderProducts(category) {
        ...
        products.forEach(p => {
            const div = document.createElement('div');
            div.className = 'product tour-card';
            div.onclick = () => { if (p.url && p.url !== '#') window.location.href = p.url + "?v=1.4"; else alert('상세 페이지 준비 중'); };
            div.innerHTML = `
                ${getBadgesHtml(p)}
                <div class="img-container card-img-wrap"><img src="${p.img}?v=1.4" alt="${p.title}" loading="lazy"></div>
                <div class="card-body">
